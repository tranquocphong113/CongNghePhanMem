...
// --- DỮ LIỆU GIẢ CHO TENANT (Để không bị lỗi giao diện) ---
...
export function AppSidebar({ className }: { className?: string }) {
  ...
  // Lấy user thật từ Service
  const user = authStorageService.getCurrentUser();

  // Khai báo tenant giả tạm thời
  ...
  // --- HÀM XỬ LÝ ĐĂNG XUẤT ---
  const handleLogout = () => {
    // 1. Xóa token (Dùng hàm clear thay vì logout)
      ...
      <div className="p-4 border-b border-sidebar-border">
        ...
        <div className="flex items-center gap-2 mb-4 px-2">
          ...
          <div>
            ...
            <p className="text-xs text-sidebar-primary/80">
              Enterprise Platform
            </p>
          </div>
        </div>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="outline"
              className="w-full justify-between bg-sidebar-accent/10 border-sidebar-accent/20 hover:bg-sidebar-accent/20 text-sidebar-foreground h-12"
            >
          ...
          <DropdownMenuTrigger asChild>
            <button className="flex items-center gap-3 w-full hover:bg-sidebar-accent/20 p-2 rounded-lg transition-colors text-left group">
              <Avatar className="h-9 w-9 border border-sidebar-border group-hover:border-sidebar-primary/50 transition-colors">
                ...
                <AvatarFallback className={user?.avatarColor || "bg-gray-500"}>
                  {user?.name?.[0] || "U"}
                </AvatarFallback>
              </Avatar>
